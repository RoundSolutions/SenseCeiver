...
//extern INT8 USBIsConnected;
//extern M2M_T_HW_USB_HANDLE global_usb;
void M2M_onUSbCableEvent(UINT32 usb_cable_event)
{
	/*if(m2m_hw_usb_cable_check()== USB_CABLE_ATTACHED)
	{
		m2m_hw_usb_open(USB_CH0,&global_usb);
		//Configure AT-Mode
		//m2m_hw_usb_ioctl(global_usb, M2M_USB_BLOCKING_SET, (INT32)M2M_HW_USB_IO_BLOCKING_RELEASE);
		//m2m_hw_usb_ioctl(global_usb, M2M_USB_AT_MODE_SET, M2M_HW_USB_IO_AT_MODE_ON);
		USBIsConnected = 1;
	}
	if(m2m_hw_usb_cable_check()== USB_CABLE_DETACHED)
	{
		USBIsConnected = 0;
		//m2m_hw_usb_ioctl(global_usb, M2M_USB_AT_MODE_SET, M2M_HW_USB_IO_AT_MODE_OFF);
		m2m_usb_close_hwch(&global_usb);
